!+++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++
!++ calculate the synchrotron radiation of e+/e- in magnetic field
!++ input: Eelec(nelec) in GeV, Felec(nelec) in cm^-3 GeV^-1, B in Gs
!++ output: Esyn(nsyn) in GeV, Fsyn(nsyn) in GeV^-1 cm^-3 s^-1 
!++ line of sight integral divided by 4pi gives the flux with unit 
!++ erg cm^-2 s^-1 sr^-1 Hz^-1
      subroutine RADIATION_SYN(Eelec,Felec,nelec,B,Esyn,Fsyn,nsyn)
      implicit real*8 (a-z)
      integer nelec,nsyn,ielec,isyn
      real*8 Eelec(nelec),Felec(nelec),Esyn(nsyn),Fsyn(nsyn)
      ME=0.511d-3
      if (abs(Esyn(nsyn)-Esyn(1))<1d-50) then
         Esyn_min=1.d-15               ! [GeV]
         Esyn_max=1.d0                 ! [GeV]
         synfactor=exp(log(Esyn_max/Esyn_min)/(nsyn-1))
         do isyn=1,nsyn
            Esyn(isyn)=Esyn_min*synfactor**(isyn-1) ! [GeV]
         enddo
      endif
      elecfactor=Eelec(nelec)/Eelec(nelec-1)
      do isyn=1,nsyn
         Esyn_Hz=Esyn(isyn)*2.418d23 ! [Hz]
         syn_emiss=0.d0
         do ielec=1,nelec
            e_lorentz=Eelec(ielec)/ME+1
            syn_emiss=syn_emiss+Felec(ielec)*
     &SYNCHROTRON(e_lorentz,Esyn_Hz,B)*Eelec(ielec)
!++ &SYNCHROTRON in erg s^-1 Hz^-1, syn_emiss in erg cm^-3 s^-1 Hz^-1
         enddo
         Fsyn(isyn)=syn_emiss*log(elecfactor)/Esyn(isyn)*1.5092d26
!++ Fsyn in [GeV^-1 cm^-3 s^-1]
      enddo
      end
!+++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++
...
!** a mistake: should be /Esynch/h/h, yuanqiang
c[2]     3   /Esynch/h/h                     !<- transformation to units 1/MeV/s
      return
      end
